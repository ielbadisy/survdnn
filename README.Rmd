...
`tune_survdnn()` can be used also to automatically refit the best-performing model on the full dataset.

This behavior is controlled by the `refit` and `return` arguments. For example:
...
The `survdnn` R package is available on [CRAN](https://cran.r-project.org/web/packages/survdnn/index.html) or [github](https://github.com/ielbadisy/survdnn)  
